...
void SynthVoice::SetCutoff(float lvl)  {
  _cutoff = lvl;
...
void SynthVoice::PitchBend(int number) {
  //
...
void SynthVoice::ParseCC(uint8_t cc_number , uint8_t cc_value) {
  float tmp = 0.0f;
  switch (cc_number) {
...
void SynthVoice::on_midi_noteON(uint8_t note, uint8_t velocity)
{
...
void SynthVoice::on_midi_noteOFF(uint8_t note, uint8_t velocity)
{
  if (mva1.n == 0) {
    return;
  }
